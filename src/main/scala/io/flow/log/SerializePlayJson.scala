...
class SerializePlayJson extends JsonFactoryDecorator {
  override def decorate(factory: MappingJsonFactory): MappingJsonFactory = {
    factory.getCodec.registerModule(PlayJsonModule): @silent //please remove once PlayJsonModule(jsonParsersettings) is accessible outside of the jackson package
    factory
  }
}
